...
## 2026-08-22 - [Discovery Target Processing and Formatting Fast-Paths]
**Learning:** In `DiscoveryService`, converting raw size arrays to float arcminutes via element-wise `getattr` loops over candidate objects creates unnecessary overhead when the underlying array is already a numeric NumPy array (`float64`). Checking `arr.dtype != object` allows returning `np.asarray(arr, dtype=float)` directly, speeding up size conversion for float arrays by >3,000x. Additionally, applying magnitude pre-filtering before Messier catalog deduplication prunes ~80% of candidate rows prior to string processing, and slicing top N results before name fallback replacement avoids string manipulations on thousands of discarded candidates.
**Action:** Always check `arr.dtype != object` before running object extraction loops on NumPy arrays, apply numerical filters before string deduplication filters, and slice top results before running result formatting fallbacks.
