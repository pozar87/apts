## 2026-08-29 - [Per-Moon Parameter Pre-calculation in Jovian Mutual State Search]
**Learning:** In Skyfield-based searches for Galilean moon mutual events (`find_jovian_mutual_events`), evaluating pairwise moon states in `JovianMutualState._compute_pair_states_sliced` inside a loop over the 6 moon pairs repeatedly computed distances (`m.distance().km`), unit direction vectors (`p / d`), and angular radii (`_get_moon_angular_radius(mid, d)`) 12 times per Earth and Sun perspective (24 times total per time step). Pre-calculating per-moon parameter dictionaries (`d_e`, `u_e`, `r_e`, `d_s`, `u_s`, `r_s`) once before entering the pairwise loop reduces vector norms, divisions, and `arcsin` evaluations from 24x to 8x per step evaluation. Yields a ~15% speedup for `find_jovian_mutual_events` (~1,562 ms down to ~1,329 ms per 30-day search) with 100% exact result identity.
**Action:** When evaluating state functions over pairs derived from a small set of objects (e.g., Galilean moons), pre-compute single-object parameters (distances, unit vectors, angular radii) before looping over pairwise combinations.

## 2026-08-28 - [Astrometric Observation Bypass in Conjunction Coarse Minima Search]
**Learning:** In Skyfield-based searches for conjunctions (`find_conjunctions`), evaluating topocentric `.apparent()` positions inside the step-search separation function (`find_minima`) triggers expensive Standard Apparent coordinate transformations (nutation, aberration, gravitational deflection) for dozens of time steps. Because step searching only identifies approximate candidate minima times (which are subsequently refined with high precision in `_refine_conjunction`), using raw astrometric `.observe()` during step searching completely bypasses nutation/aberration overhead, yielding a ~20-25% speedup (~1,022 ms down to ~792 ms for 1-year planet solar conjunctions) with 100% exact result identity.
**Action:** Always bypass expensive `.apparent()` coordinate transformations in favor of raw astrometric `.observe()` during step-searching coarse minima functions when candidate refinement logic handles high-precision evaluation.

## 2026-08-27 - [Jovian Moon & Mutual Event Pre-Sliced Observation Arrays & Direct Elongation Comparison]
**Learning:** In Skyfield-based searches for Galilean moon events (`find_jovian_moon_events` and `find_jovian_mutual_events`), state functions evaluate states across time arrays `t`. Slicing `t` (`t[visible]`) creates a new Skyfield `Time` object that discards cached nutation angles (`_nutation_angles_radians`). As a result, subsequent calls to `ctx.get_basic_data(t[visible])` and `ctx.get_obs_at_t(t[visible])` trigger expensive topocentric frame rotations and `iau2000a` nutation recalculations from scratch. By extracting observations (`j_obs`, `sun_from_j`, `m_obs`, `moons_e`, `moons_s`) for the full time vector `t` first and slicing the observation arrays directly with the `visible` boolean mask (`_compute_vector_sliced` and `_compute_pair_states_sliced`), we completely eliminate redundant Skyfield observer evaluations and `iau2000a` nutation calculations. Furthermore, replacing `j_obs.separation_from(s_obs).degrees > 10` in `_compute_full_visibility` with direct unit vector dot product comparison (`dot_js < np.cos(np.radians(10))`) avoids Skyfield `Angle` object allocation in tight loops. Together, these changes speed up `find_jovian_moon_events` from ~2.63s to ~1.85s (~1.44x speedup) and `find_jovian_mutual_events` from ~1.33s to ~1.20s, saving >0.90s per 30-day run with 100% exact event identity.
**Action:** When evaluating state functions over filtered/visible subsets of Skyfield `Time` arrays, compute observation position arrays over the full `Time` array once and slice the resulting position arrays directly with the boolean mask instead of slicing the `Time` object, avoiding redundant topocentric frame setup and `iau2000a` nutation re-computations.

## 2026-08-27 - [Astrometric Observation Bypass for Moon Separation and Bright Limb Angle]
**Learning:** In `get_moon_separation` and `get_moon_position_angle_bright_limb` (`apts/utils/planetary/moon.py`), calling `.apparent()` on topocentric/geocentric observations (`observer.at(time).observe(obj).apparent()`) triggers expensive Skyfield nutation (`iau2000a`), gravitational deflection, and aberration calculations for every observation call. Because relative angular separation between celestial targets and the Moon (or relative position angle of the Moon's bright limb relative to the Sun) measures 3D vector orientation from the observer, applying `.apparent()` to both vectors rotates them identically by tiny arcseconds without meaningfully affecting angular separation (~0.01° difference). Replacing `.apparent()` with raw astrometric `.observe()` calls bypasses redundant nutation/aberration calculations, yielding an ~18x speedup (~2.0ms down to ~0.1-0.5ms per call) during target scoring and lunar calculations.
**Action:** Always use raw astrometric `.observe()` instead of `.apparent()` when evaluating relative angular separations or orientation angles between celestial targets measured from the same observer.

## 2026-08-26 - [Geocentric ICRF Observation Bypass for Star/Messier Conjunction Candidate Search]
**Learning:** In `find_conjunctions_with_stars` (`apts/skyfield_searches/conjunctions/stars.py`), evaluating topocentric `.apparent()` positions across thousands of time steps (`observer.at(times).observe(body).apparent()`) when pre-computed positions are absent forces expensive Skyfield topocentric frame rotations and `iau2000a` nutation angle calculations for every timestamp. Because coarse candidate identification only checks angular separation threshold local minima against fixed stars in ICRF, topocentric frame rotation and nutation are redundant. Replacing this with geocentric ICRF observations (`earth.at(times).observe(body)` and `earth.at(t_mid).observe(stars_vector)`) completely bypasses nutation/GAST calculations during coarse filtering. High-precision topocentric observations and iterative solver minimization in `_refine_conjunction` remain untouched for candidate refinement, providing a ~7.5x speedup for `find_planet_messier_conjunctions` (~5.08s down to ~0.67s) and ~13x speedup for `find_planet_star_conjunctions` (~4.04s down to ~0.31s).
**Action:** When performing coarse candidate searches over time step grids between moving bodies and fixed catalog objects, evaluate positions in geocentric ICRF (`earth.at(times).observe(body)`) to bypass topocentric frame transformation and nutation overhead, reserving topocentric `.apparent()` calculations for candidate refinement.

## 2026-08-25 - [Discovery Service Fast-Path Target Processing Optimization]
**Learning:** In `DiscoveryService.get_top_picks`, processing targets across large catalogs (including 14,000+ NGC rows) suffered from three bottlenecks: (1) element-wise `getattr` loop overhead in `_extract_arcmin_floats` when handling float arrays, (2) applying Messier catalog string deduplication (`Name_norm`, `NGC_norm`, `IC_norm`) across all 14,000+ rows before magnitude filtering, and (3) applying `Name` fallback logic and dictionary formatting to the full target set before slicing top results. Hoisting magnitude filtering (`Magnitude_float <= ngc_magnitude_limit`) before deduplication prunes ~80% of rows (~14,000 down to ~2,700) prior to string processing, adding a direct non-object array check in `_extract_arcmin_floats` bypasses element-wise inspection, and slicing `head(limit)` in `_format_discovery_results` before dictionary formatting avoids formatting non-returned candidates. Together these optimizations provide a ~1.70x speedup for `DiscoveryService.get_top_picks` (~553ms down to ~325ms) with 100% exact result identity.
**Action:** Always hoist simple numeric threshold filters (like magnitude limits) ahead of string normalization and deduplication loops over large catalog DataFrames. Slice top N result DataFrames before applying dictionary transformation and fallback logic.

## 2026-08-24 - [Vectorized Coarse Separation Pre-filtering for Planet-Planet Occultations]
**Learning:** In planet-planet occultation searches (`find_planet_planet_occultations`), running `find_minima` step-searches for all 21 planet pairs over long time windows executes hundreds of Skyfield position evaluations per pair, even though planet-planet occultations and sub-degree approach events are extremely rare. Pre-evaluating topocentric positions for all 7 planets on a coarse 0.5-day grid in a single vectorized Skyfield pass (`obs_at_coarse = observer.at(t_coarse)`) and skipping `find_minima` for pairs whose minimum separation exceeds 1.0 degree prunes non-candidate pairs before solver initialization, cutting runtime from ~608ms to ~14ms per call (~43x speedup).
**Action:** Always pre-calculate coarse positions for multi-body pairwise searches in a single vectorized observation pass and prune non-candidate pairs before running iterative solvers like `find_minima`.

## 2026-08-23 - [Vectorized Celestial Object AltAz in Finder Chart Drawing]
**Learning:** In finder chart rendering, iterating row-by-row over catalog DataFrames (`for _, row in stars_df.iterrows()`) to observe and compute AltAz for bright stars and Messier objects triggers ~160 individual Skyfield `.observe().apparent()` calls and heavy Pandas Series object allocations per chart. Replacing this with pre-cached vectorized `Star` objects (`Star(ra_hours=df['ra_hours'].values, dec_degrees=df['dec_degrees'].values)`) and evaluating topocentric coordinates in a single `fast_altaz` call yields a ~23x speedup for background celestial object coordinate computation (~266ms down to ~11.6ms per chart) and speeds up overall finder chart generation by ~1.67x (~637ms down to ~382ms per chart).
**Action:** When rendering background stars or catalog objects on charts or skymaps, pre-cache array-backed `Star` objects for the catalog and perform a single vectorized `fast_altaz` observation instead of row-by-row iteration.

## 2026-08-22 - [Two-Stage Geocentric/Topocentric Lunar Occultation Filtering]
**Learning:** Evaluating topocentric observer state (`observer.at(coarse_times)`) in Skyfield over dense time grids forces expensive ITRS frame rotations, GAST, and IAU2000 nutation calculations for every time step. Since topocentric lunar parallax is bounded by < 1.05°, performing an initial geocentric Moon position check (`earth.at(coarse_times).observe(moon)`) with a 1.3° safety margin filters out >95% of non-candidate coarse times instantly. Performing full topocentric observations only for candidate times yields a ~5.3x speedup for 1-year lunar occultation searches (~1,732 ms down to ~321 ms) with 100% exact result identity.
**Action:** When performing coarse spatial filtering for moving bodies with bounded parallax (e.g., Moon), use geocentric positions with a parallax safety margin to prune empty time windows before evaluating topocentric observer positions.
...
## 2026-07-23 - [Direct Value List Iteration for String Coordinate & Catalog Parsing]
**Learning:** When parsing structured string columns in Pandas DataFrames (such as 'HH:MM:SS' RA or '+/-DD:MM:SS' Dec coordinates and catalog names in large catalogs like NGC with 14,000+ entries), relying on Pandas `.str` methods (`.str.split(expand=True)`, `.str.extract`, `.str.replace`) and `pd.to_numeric` across multi-column Series introduces significant Series alignment and conversion overhead. Replacing these with a direct list comprehension / `for` loop over `series.values` yields a ~2.5x speedup for parsing catalog coordinates and normalized names while maintaining 100% precision and handling edge cases (like `-00` degree declination strings).
**Action:** Prefer direct list iteration over `.values` for structured string parsing and regex normalization on large string Series when vectorization methods incur heavy Pandas object overhead.
...
## 2026-08-19 - [DatetimeIndex Optimization in Vectorized Astronomical Calculations]
**Learning:** Wrapping `pd.DatetimeIndex` objects in `pd.Series` and using `.astype(object).where(...)` to filter invalid values causes heavy Pandas Series allocation, object-dtype coercion, and timezone-inference overhead during high-frequency catalog computes (e.g. `NGC.compute` with 14,000+ objects). Operating directly on `pd.DatetimeIndex` and converting to Python datetimes via `np.array(dti.tz_convert(tz).to_pydatetime(), dtype=object)` with NumPy boolean masking (`arr[~valid_mask] = None`) bypasses Pandas Series overhead for a ~1.3x speedup (81ms down to 60ms).
**Action:** Prefer direct `pd.DatetimeIndex` operations and NumPy array masking over `pd.Series.astype(object).where(...)` when formatting time arrays to Python list outputs.

## 2026-08-20 - [Geometric Ecliptic Latitude Masking]
**Learning:** For fixed catalog objects (stars, Messier objects), filtering candidates by ecliptic latitude using Skyfield's topocentric `observer.at(t_ref).observe(star_vector).ecliptic_latlon()` triggers expensive frame transformations, nutation, and aberration calculations for thousands of targets. Calculating ecliptic latitude geometrically using J2000 obliquity constants ($\sin\beta = \text{sin\_dec} \cdot \cos\epsilon - \text{cos\_dec\_sin\_ra} \cdot \sin\epsilon$) with pre-calculated direction cosines yields mathematically identical results ($\sim 10^{-15}$ epsilon) while providing a ~30-115x speedup per filtering step.
**Action:** Use geometric ecliptic latitude masking with pre-calculated direction cosines (`sin_dec`, `cos_dec_sin_ra`) for candidate pre-filtering of fixed catalog objects near the ecliptic.

## 2026-08-21 - [Lunar Libration & Colongitude Astrometric Observe & fast_altaz Optimization]
**Learning:** In Skyfield, evaluating lunar libration (`get_moon_libration`) and colongitude (`get_moon_colongitude`) using `.observe(...).apparent()` applies expensive nutation (`iau2000a`) and aberration calculations that are unnecessary because selenographic calculations are subsequently performed in the ICRF reference frame. Replacing `.apparent()` with raw astrometric `.observe(...)` and using `fast_altaz` for topocentric visibility checks in `find_moon_libration_maxima` yields a ~1.27x speedup (~0.78s to ~0.61s) with identical results.
**Action:** Use astrometric `.observe(...)` instead of `.apparent()` when computing selenographic orientation vectors (libration, colongitude) and use `fast_altaz` for fast visibility checks.
