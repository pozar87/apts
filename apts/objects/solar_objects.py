...
class SolarObjects(Objects):
    def __init__(self, place):
        ...
        # Add name
        self.objects[ObjectTableLabels.EPHEM] = self.objects[
    ...
    def compute(self, calculation_date=None):
        ...
        self.objects[ObjectTableLabels.MAGNITUDE] = self.objects[
            [ObjectTableLabels.EPHEM]
        ].apply(lambda body: almanac.oppositions_conjunctions(self.eph, body.Ephem)(t) if body.name != 'Sun' and body.name != 'Moon' else 0, axis=1)
        # Calculate planets RA
        self.objects[ObjectTableLabels.RA] = self.objects[
            [ObjectTableLabels.EPHEM]
        ].apply(
            lambda body: body.Ephem.at(t).observe(self.place.observer).radec()[0].hours, axis=1
        )
        # Calculate planets Dec
        self.objects[ObjectTableLabels.DEC] = self.objects[
            [ObjectTableLabels.EPHEM]
        ].apply(lambda body: body.Ephem.at(t).observe(self.place.observer).radec()[1].degrees, axis=1)
        # Calculate planets distance from Earth
        self.objects[ObjectTableLabels.DISTANCE] = self.objects[
            [ObjectTableLabels.EPHEM]
        ].apply(lambda body: body.Ephem.at(t).observe(self.place.observer).distance().au, axis=1)
        # Calculate planets size
        self.objects[ObjectTableLabels.SIZE] = self.objects[
            [ObjectTableLabels.EPHEM]
        ].apply(lambda body: body.Ephem.radius.km if hasattr(body.Ephem, 'radius') else 0, axis=1)
        # Calculate planets elongation
        self.objects[ObjectTableLabels.ELONGATION] = self.objects[
            [ObjectTableLabels.EPHEM]
        ].apply(lambda body: body.Ephem.at(t).observe(self.place.observer).separation_from(self.place.sun.at(t)).degrees, axis=1)
        # Calculate planets phase
        self.objects[ObjectTableLabels.PHASE] = self.objects[
            [ObjectTableLabels.EPHEM]
        ].apply(lambda body: body.Ephem.at(t).observe(self.place.observer).phase_angle(self.place.sun).degrees, axis=1)
