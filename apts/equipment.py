...
class Equipment:
  ...
  def data(self):
    columns=['label','type','zoom','useful_zoom','fov','range','brightness','elements']
    def append(result_data, columns, paths):
      ...
      return result_data 
    result_data = pd.DataFrame(columns=columns) #TODO: This is still not best way to add data 
    result_data = append(result_data, columns, self._get_paths(Constants.EYE_ID))
    result_data = append(result_data, columns, self._get_paths(Constants.IMAGE_ID))
    return result_data    
  
  def plot(self, to_plot):
    data = self.data()[[to_plot,'type','label']].sort_values(by=to_plot)
    pd.DataFrame([{row[1]:row[0]} for row in data.values], index=data['label'].values).plot(kind="bar", stacked = True, title=to_plot)
