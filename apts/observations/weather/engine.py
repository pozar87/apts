...
def _get_hour_analysis_reasons(
    row: dict, idx: int, masks: dict[str, pd.Series], conditions: Conditions
) -> tuple[list[str], list[str]]:
    """Generates localized reasons and keys for a single bad weather hour."""
    ...
    # atmospheric checks
    _check_condition(
        masks["is_bad_clouds"].iloc[idx],
        "BAD_CLOUDS",
        gettext_("Cloud cover %(cloud_cover)s%% exceeds limit of %(max_clouds)s%%"),
        {"cloud_cover": f"{row['cloudCover']:.1f}", "max_clouds": conditions.max_clouds},
        reasons,
        reason_keys,
    )
    _check_condition(
        masks["is_bad_precip_prob"].iloc[idx],
        "BAD_PRECIP",
        gettext_(
    ...
    _check_condition(
        masks["is_bad_precip_intens"].iloc[idx],
        "BAD_PRECIP",
        gettext_(
    ...
    _check_condition(
        masks["is_bad_wind"].iloc[idx],
        "BAD_WIND",
        gettext_("Wind speed %(wind_speed)s km/h exceeds limit of %(max_wind)s km/h"),
        {"wind_speed": f"{row['windSpeed']:.1f}", "max_wind": conditions.max_wind},
        reasons,
        reason_keys,
    )
    _check_condition(
        masks["is_bad_temp"].iloc[idx],
        "BAD_TEMP",
    ...
    _check_condition(
        masks["is_bad_vis"].iloc[idx],
        "BAD_VIS",
        gettext_("Visibility %(vis)s km below limit of %(min_vis)s km"),
        {"vis": f"{row['visibility']:.1f}", "min_vis": conditions.min_visibility},
        reasons,
        reason_keys,
    )
    _check_condition(
        masks["is_bad_fog"].iloc[idx],
        "BAD_FOG",
        gettext_("Fog %(fog)s%% exceeds limit of %(max_fog)s%%"),
        {"fog": f"{row['fog']:.1f}", "max_fog": conditions.max_fog},
        reasons,
        reason_keys,
    )
    _check_condition(
        masks["is_bad_moon"].iloc[idx],
        "BAD_MOON",
        gettext_(
            "Moon illumination %(illum)s%% exceeds limit of %(max_illum)s%% while moon is up"
        ),
        {"illum": f"{row['moonIllumination']:.1f}", "max_illum": conditions.max_moon_illumination},
        reasons,
        reason_keys,
    )
    _check_condition(
        masks["is_bad_aurora"].iloc[idx],
        "BAD_AURORA",
        gettext_("Aurora %(aurora)s%% below limit of %(min_aurora)s%%"),
        {"aurora": f"{row['aurora']:.1f}", "min_aurora": conditions.min_aurora},
        reasons,
        reason_keys,
    )
    _check_condition(
        masks["is_bad_seeing"].iloc[idx],
        "BAD_SEEING",
        gettext_("Seeing %(seeing)s arcsec exceeds limit of %(max_seeing)s arcsec"),
        {"seeing": f"{row['seeing']:.1f}", "max_seeing": conditions.max_seeing},
        reasons,
        reason_keys,
    )
    _check_condition(
        masks["is_bad_sqm"].iloc[idx],
        "BAD_SQM",
        gettext_(
            "Sky brightness %(sqm)s mag/arcsec² below limit of %(min_sqm)s mag/arcsec²"
        ),
        {"sqm": f"{row['sqm']:.1f}", "min_sqm": conditions.min_sqm},
        reasons,
        reason_keys,
    )

    return reasons, reason_keys
...
def _check_condition(
    ...
    """Helper to append a reason if a condition is met."""
    if is_bad:
        reason_keys.append(key)
        reasons.append(message_template % template_args)
    ...
    with language_context(language):
        ...
        if len(bad_indices) > 0:
            ...
            for i, row in enumerate(bad_rows):
                idx = bad_indices[i]
                reasons, reason_keys = _get_hour_analysis_reasons(
                    row, idx, masks, conditions
                )
