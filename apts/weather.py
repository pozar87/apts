...
class Weather:
  ...
  def plot_precip(self, hours = 24, **args):  
    ...
    plt = data.plot(x = "time", title = "Precip intensity and probability", **args)
    return plt
     
  def plot_precip_type_summary(self, hours = 24, **args):
    data = self._filter_data(["precipType"], hours)
    plt = data.groupby('precipType').size().plot(kind = 'pie', label = "Precip type summary", **args)
    return plt    
    
  def plot_clouds_summary(self, hours = 24, **args):
    data = self._filter_data(["summary"], hours)
    plt = data.groupby('summary').size().plot(kind = 'pie', label = "Cloud summary", **args)
    return plt   
  
  def plot_temperature(self, hours = 24, **args):
    data = self._filter_data(["temperature", "apparentTemperature", "dewPoint"], hours)
  ...
  def plot_wind(self, hours = 24, **args):
    ...
    plt = data.plot(x = "time", y = "windSpeed", ylim=(0, max_wind_speed  + 1), title = "Wind speed", **args)
    return plt    
  
  def plot_pressure_and_ozone(self,  hours = 24, **args):
    data = self._filter_data(["pressure","ozone"], hours)
    plt = data.plot(x = "time", title="Pressure and Ozone", secondary_y = ['ozone'],  **args)
    return plt  
  
  def get_critical_data(self, start, stop):
    data = self._filter_data(["cloudCover","precipProbability","windSpeed","temperature"], hours = 24)
    return data[(data.time > start) & (data.time < stop)] 
     
  def plot_visibility(self, hours = 24, **args):
    data = self._filter_data(["visibility"], hours)
    plt = data[data.visibility != 'none'].plot(x = "time", title = "Visibility", **args)
    return plt
  ...
  def download_data(self):  
    ...
    with urllib.request.urlopen(Weather.API_URL.format(Weather.API_KEY, self.lat, self.lon)) as url:
      ...
      raw_data = [[(item[column] if column in item.keys() else 'none') for column in columns] for item in json_data["hourly"]["data"]]
