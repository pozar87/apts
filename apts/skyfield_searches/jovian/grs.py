...
def _get_cml_difference(t, grs_longitude):
...
def _get_abs_diff(t, grs_longitude):
    return np.abs(_get_cml_difference(t, grs_longitude))
...
def find_jupiter_grs_transits(
    ...
    def state_func(t):
        return _get_abs_diff(t, grs_longitude)

    setattr(state_func, "step_days", 0.1)
    times, _ = find_minima(t0, t1, state_func)
