...
class PolarAlignment:
    ...
    def _robust_star_tracking(self, prev_target, prev_stars, current_stars):
        ...
        neighbors = self._get_target_neighbors(prev_target, prev_stars)
        if not neighbors:
            return None

        # Precompute relative distances from target to neighbors in previous frame
        target_to_neighbors_dists = [self._dist(prev_target, n) for n in neighbors]
        ...
        for s_curr in current_stars:
            match_count = self._match_asterism(
    ...
    def _get_target_neighbors(self, prev_target, prev_stars):
        ...
        neighbors = sorted(
            [
                s
                for s in prev_stars
                if self._dist(s, prev_target) > 1.0 and s.get("flux", 0) > min_flux
            ],
            key=lambda s: self._dist(s, prev_target),
        )[:5]  # Take up to 5 closest neighbors
        return neighbors

    def _match_asterism(
        self, s_curr, prev_target, target_to_neighbors_dists, current_stars
    ):
        ...
        curr_neighbors_dists = sorted(
            [
                self._dist(s_curr, other)
                for other in current_stars
                if self._dist(s_curr, other) > 1.0
            ]
        )
    ...
    def _fit_circle(self):
        ...
        points = np.array(
            [(f["target"]["x"], f["target"]["y"]) for f in self.rotation_frames]
        )
    ...
    def _estimate_sensor_params(self):
        ...
        p1 = np.array(
            [self.rotation_frames[0]["target"]["x"], self.rotation_frames[0]["target"]["y"]]
        )
    ...
    def calculate_correction(self):
        ...
        ncp_image = (
            p_current
            + (d_alt_sky * 3600 / self.pixel_scale) * u_alt
        ...
        if abs(err_alt) > 1 / 60.0:
            inst.append(
                f"Move Altitude {'Down' if err_alt > 0 else 'Up'} {abs(err_alt*60):.1f}'"
            )
        if abs(err_az) > 1 / 60.0:
            inst.append(
                f"Move Azimuth {'West' if err_az > 0 else 'East'} {abs(err_az*60):.1f}'"
            )
    ...
    def generate_alignment_map(self, dark_mode=True):
        ...
        ax.scatter(
            axis[0], axis[1], c="red", marker="x", s=100, label="Mount RA Axis", zorder=5
        )
        ...
        ax.set_title(
            f"Polar Alignment Guide - Error: {correction['error_arcmin']:.1f}'",
            color=text_color,
        )
