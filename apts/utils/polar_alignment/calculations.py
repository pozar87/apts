...
def _compute_parallactic_angle(lat, star_alt, star_dec, t, lon_decimal, star_ra_hours):
    lat_rad = np.radians(lat)
    alt_rad = np.radians(star_alt)
    dec_rad = np.radians(star_dec)

    lst = t.gmst + lon_decimal / 15.0
    h_rad = np.radians((lst - star_ra_hours) * 15.0)
...
def _compute_image_orientation(p_current, c, q):
...
def _compute_error_vector(
    p_current, c, star_alt, star_az, pole_alt, pole_az, pixel_scale, u_alt, u_az
):
    ...
    return ncp_image, err_alt, err_az, total_error_arcmin


def _generate_correction_instructions(err_alt, err_az):
    inst = []
...
def calculate_correction(
    ...
    latest_frame = adjustment_frames[-1] if adjustment_frames else rotation_frames[-1]
    ...
    q = _compute_parallactic_angle(
        lat,
        star_alt,
        target_star.dec.degrees,
        t,
        observation.place.lon_decimal,
        target_star.ra.hours,
    )

    p_current = np.array([latest_frame["target"]["x"], latest_frame["target"]["y"]])
    c = mount_axis_image
    u_alt, u_az = _compute_image_orientation(p_current, c, q)

    ncp_image, err_alt, err_az, total_error_arcmin = _compute_error_vector(
        p_current, c, star_alt, star_az, pole_alt, pole_az, pixel_scale, u_alt, u_az
    )

    instructions = _generate_correction_instructions(err_alt, err_az)
