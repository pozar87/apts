...
class TestCircumpolarObjects(unittest.TestCase):
    def test_circumpolar_messier_object_visibility(self):
        ...
        place = Place(
            lat=69.6,
            lon=18.9,
            date=datetime(2025, 3, 20, 22, 0, 0, tzinfo=timezone.utc),
        )
        equipment = Equipment()
        ...
        obs = Observation(
            place, equipment, conditions, target_date=datetime(2025, 3, 20).date()
        )
        ...
        self.assertTrue(
            m81_is_visible, "M81 should be visible as a circumpolar object from Tromsø."
        )
    ...
    def test_circumpolar_below_horizon_messier_object_is_not_visible(self):
        ...
        place = Place(
            lat=69.6,
            lon=18.9,
            date=datetime(2025, 6, 20, 22, 0, 0, tzinfo=timezone.utc),
        )
        equipment = Equipment()
        conditions = Conditions()
        ...
        obs = Observation(
            place, equipment, conditions, target_date=datetime(2025, 6, 20).date()
        )
        ...
        self.assertFalse(
            m7_is_visible,
            "M7 should not be visible from Tromsø as it is always below the horizon.",
        )
