...
class TestNasaAPI(unittest.TestCase):
    ...
    def test_get_comets(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            self.assertEqual(data["near_earth_objects"]["2023-01-01"][0]["name"], "Asteroid 1")
    ...
    def test_get_comets_multi_page(self):
        ...
        mock_response_2 = {
            "near_earth_objects": {
                "2023-01-08": [{"name": "Asteroid 2"}]
            }
        }

        with requests_mock.Mocker() as m:
            # Match by params start_date
            m.get("https://api.nasa.gov/neo/rest/v1/feed?start_date=2023-01-01&end_date=2023-01-07&api_key=TEST_KEY", json=mock_response_1)
            m.get("https://api.nasa.gov/neo/rest/v1/feed?start_date=2023-01-08&end_date=2023-01-10&api_key=TEST_KEY", json=mock_response_2)
