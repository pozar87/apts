...
def test_plot_connection_graph_background_color():
    ...
    assert actual_rgba == pytest.approx(expected_rgba), f"Expected {expected_rgba}, got {actual_rgba}"
    ...
    assert actual_rgba_light == pytest.approx(expected_rgba_light), f"Expected {expected_rgba_light}, got {actual_rgba_light}"
...
def test_plot_connection_graph_svg_contains_color():
    ...
    assert expected_facecolor_hex in svg_data.lower(), f"SVG should contain {expected_facecolor_hex}"
