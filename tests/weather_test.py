...
def test_download_data_key_error_handling(requests_mock):
    ...
    assert df.empty
    assert all(col in df.columns for col in ["time", "summary", "precipType", "precipProbability", "precipIntensity", "temperature", "apparentTemperature", "dewPoint", "humidity", "windSpeed", "cloudCover", "visibility", "pressure", "ozone"])

def test_plot_weather_calls_sub_plots(requests_mock):
    mock_api_response = {
        "hourly": {
            "data": [
                {"time": 1624000000 + i * 3600, "cloudCover": 0.1, "precipProbability": 0.05, "windSpeed": 5, "temperature": 15, "summary": "Clear", "precipType": "none", "apparentTemperature": 14, "dewPoint": 10, "humidity": 0.7, "visibility": 10, "pressure": 1012, "ozone": 300}
                for i in range(24)
            ]
        }
    }
    ...
    obs = Observation(place=real_place, equipment=mock_equipment, conditions=mock_conditions)
    ...
    with (
        ...
        patch.object(mock_weather_instance, 'plot_clouds_summary') as mock_plot_clouds_summary,
        patch.object(mock_weather_instance, 'plot_precipitation') as mock_plot_precipitation,
        patch.object(mock_weather_instance, 'plot_precipitation_type_summary') as mock_plot_precipitation_type_summary,
        patch.object(mock_weather_instance, 'plot_temperature') as mock_plot_temperature,
        patch.object(mock_weather_instance, 'plot_wind') as mock_plot_wind,
        patch.object(mock_weather_instance, 'plot_pressure_and_ozone') as mock_plot_pressure_and_ozone,
        patch.object(mock_weather_instance, 'plot_visibility') as mock_plot_visibility,
        patch('apts.observations.pyplot.subplots', return_value=(MagicMock(), MagicMock(shape=(4,2)))) as mock_subplots,
        patch('apts.observations.Observation._mark_observation') as mock_mark_observation,
        patch('apts.observations.Observation._mark_good_conditions') as mock_mark_good_conditions,
        patch('apts.observations.Observation.plot_sun_and_moon_path') as mock_plot_sun_and_moon_path
    ):
