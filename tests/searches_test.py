...
class SearchesTest(unittest.TestCase):
    ...
    def test_find_aphelion_perihelion(self):
        events = searches.find_aphelion_perihelion(self.eph, 'mars', self.start_date, self.end_date)
    ...
    def test_find_lunar_occultations(self):
        ...
        events = searches.find_lunar_occultations(self.observer, self.eph, sirius, short_start, short_end)
        self.assertIsInstance(events, list)

    def test_find_mercury_inferior_conjunctions(self):
        start_date = datetime(2023, 1, 1, tzinfo=utc)
        end_date = datetime(2023, 12, 31, tzinfo=utc)
        events = searches.find_mercury_inferior_conjunctions(self.eph, start_date, end_date)
        self.assertGreater(len(events), 0)

    def test_find_moon_apogee_perigee(self):
        events = searches.find_moon_apogee_perigee(self.eph, self.start_date, self.end_date)
    ...
    def test_find_conjunctions(self):
        ...
        events = searches.find_conjunctions(self.eph, 'venus', 'jupiter barycenter', start_date, end_date)
        self.assertGreater(len(events), 0)
