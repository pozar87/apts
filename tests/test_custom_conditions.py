    ...
    with patch("apts.weather.get_weather_settings") as mock_settings:
        ...
        with patch("apts.weather.PirateWeather") as mock_pw:
            ...
            mock_data = pd.DataFrame({
                "time": [datetime.datetime(2023, 6, 1, 12 + i, tzinfo=pytz.utc) for i in range(5)],
...
def test_get_weather_analysis_with_custom_conditions(mock_weather_data):
    ...
    obs = Observation(place=place, equipment=MagicMock(), conditions=standard_conditions)
...
def test_plot_weather_passes_custom_conditions(mock_plot_weather, mock_weather_data):
    ...
    custom_conditions = Conditions(max_clouds=50)
    obs.plot_weather(conditions=custom_conditions)

    # Check that plot_weather was called with custom_conditions
    args, kwargs = mock_plot_weather.call_args
    assert kwargs["conditions"] == custom_conditions
