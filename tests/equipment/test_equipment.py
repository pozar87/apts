...
def test_camera_path_with_setup_equipment():  # Renamed
    ...
    for _, data in e.connection_garph.nodes(data=True):
        ...
        if (
            isinstance(node_equipment_attr, Telescope)
            and node_equipment_attr.focal_length.magnitude == 750
            and node_equipment_attr.aperture.magnitude == 150
        ):
    ...
    for op in all_image_ops:
        ...
        if (
            isinstance(op.telescope, Telescope)  # Changed to use Telescope directly
            and op.telescope.focal_length.magnitude
            == 750  # Check properties of setup_tele
            and isinstance(op.output, Camera)  # Changed to use Camera directly
            and op.output.vendor == cam_vendor
            and not op.barlows  # Ensure no barlows
            and op.length() == 2
        ):
            target_op = op
            break
    ...
    assert target_op.zoom().magnitude == pytest.approx(
        15
    ...
    for _, row in image_paths_df.iterrows():
        if (
            cam_vendor in row[EquipmentTableLabels.LABEL]
            and setup_tele.vendor
            in row[EquipmentTableLabels.LABEL]  # "unknown telescope"
            and row[EquipmentTableLabels.ELEMENTS] == 2
        ):
            assert row[EquipmentTableLabels.ZOOM] == pytest.approx(15)
            found_in_df = True
            break
    assert found_in_df, (
        "Path not found or zoom incorrect in DataFrame for the specific Camera path"
    )
...
def test_binoculars_instantiation():
    ...
    assert bino.dawes_limit().magnitude == pytest.approx(
        11.6 / 5.0
    )  # 11.6 / 5.0 cm = 2.32
...
def test_binoculars_registration_and_graph():
    ...
    bino = Binoculars(
        magnification=10, objective_diameter=50, vendor="TestBino", apparent_fov_deg=65
    )
    eq.register(bino)
    ...
    for _, data in eq.connection_garph.nodes(data=True):
        ...
        if node_name.startswith(bino_internal_id + "_") and (
            ...
            pytest.fail(
                f"Binoculars created an unexpected connection node: {node_name}"
            )
...
def test_binoculars_in_equipment_data():
    ...
    assert len(data_df) == 2, (
        f"Expected 2 optical paths (NakedEye, Bino), got {len(data_df)}"
    )
    ...
    assert bino_row[EquipmentTableLabels.EXIT_PUPIL] == pytest.approx(
        expected_exit_pupil
    )

    expected_dawes = round(
        11.6 / 4.2, 3
    )  # Dawes: 11.6 / 4.2cm, rounded to 3 decimal places
    ...
    expected_brightness = (
        expected_exit_pupil / 7.0
    ) ** 2 * 100  # Brightness: (5.25mm / 7mm)^2 * 100
    assert bino_row[EquipmentTableLabels.BRIGHTNESS] == pytest.approx(
        expected_brightness
    )
...
def test_binoculars_do_not_connect_with_telescope_equipment():
    ...
    for index, row in data_df.iterrows():
        ...
        if "TestBino" in label:
            assert elements == 1, (
                f"Binocular path '{label}' should have 1 element, got {elements}"
            )
            assert "unknown telescope 150/750" not in label, (
                "Binocular path should not include Telescope parts"
            )
            assert "unknown ocular f=25" not in label, (
            ...
            assert "TestBino" not in label, (
                "Telescope path should not include Binocular parts"
            )
            found_tele_eyepiece_path = True

    assert found_bino_only_path, "Did not find a dedicated optical path for binoculars"
    assert found_tele_eyepiece_path, (
        "Did not find the original telescope path from setup_equipment"
    )

    bino_paths_count = sum(
        ["TestBino" in label for label in data_df[EquipmentTableLabels.LABEL]]
    )
...
def test_telescope_to_camera_direct_t2():
    ...
    target_op = None
    for op in all_image_ops:
        if tele.vendor in op.label() and cam.vendor in op.label() and op.length() == 2:
            target_op = op
            break

    assert target_op is not None, (
        "Could not find the specific Telescope->Camera OpticalPath"
    )
    ...
    expected_zoom = (
        tele.focal_length.magnitude / target_op.output._zoom_divider().magnitude
    )
    assert target_op.zoom().magnitude == pytest.approx(expected_zoom)
...
def test_telescope_barlow_t2_camera():
    ...
    for op in all_image_ops:
        if (
            tele.vendor in op.label()
            and barlow_t2.vendor in op.label()
            and cam.vendor in op.label()
            and op.length() == 3
        ):
            target_op = op
            break
    ...
    assert target_op is not None, (
        "Could not find the specific Telescope->Barlow->Camera OpticalPath"
    )
...
def test_telescope_std_barlow_t2_camera_variation():
    ...
    for _, row in image_paths.iterrows():
        if (
            ...
            and barlow.vendor in row[EquipmentTableLabels.LABEL]
            and cam.vendor in row[EquipmentTableLabels.LABEL]
            and row[EquipmentTableLabels.ELEMENTS] == 3
        ):
            correct_path_found = True
            break
    assert correct_path_found, (
        "Specific Telescope (std) -> Barlow (T2 out) -> Camera path not found or element count incorrect"
    )
    ...
    for op_path in (
        all_image_ops
    ):  # Renamed op to op_path to avoid conflict with outer scope if any
        # Check label and number of elements to identify the correct path
        if (
            tele.vendor in op_path.label()
            and barlow.vendor in op_path.label()
            and cam.vendor in op_path.label()
            and op_path.length() == 3
        ):
            target_op = op_path
            break

    assert target_op is not None, (
        "Could not find the specific Telescope->Barlow->Camera OpticalPath for detailed check"
    )
...
def test_connection_specificity_tele_no_t2_to_t2_camera():
    ...
    for _, row in image_paths.iterrows():
        if (
            ...
            and cam_t2_only.vendor in row[EquipmentTableLabels.LABEL]
            and row[EquipmentTableLabels.ELEMENTS] == 2
        ):
...
def test_connection_specificity_barlow_no_t2_to_t2_camera():
    ...
    for _, row in image_paths.iterrows():
        is_problem_path = (
            tele.vendor in row[EquipmentTableLabels.LABEL]
            and barlow_no_t2.vendor in row[EquipmentTableLabels.LABEL]
            and cam_t2_only.vendor in row[EquipmentTableLabels.LABEL]
            and row[EquipmentTableLabels.ELEMENTS] == 3  # Path with all three
        )
        if is_problem_path:
...
@patch("apts.equipment.plotting.graph.get_dark_mode")
@patch("apts.equipment.plotting.graph.nx.draw")  # Mock the nx.draw call
...
@patch("apts.equipment.plotting.graph.get_dark_mode")
@patch("apts.equipment.plotting.graph.nx.draw")
