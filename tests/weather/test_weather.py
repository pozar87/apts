...
@patch("apts.plotting.weather.utils.get_dark_mode")
@patch("apts.weather.base.get_weather_settings")
...
def test_plot_weather_calls_sub_plots(mock_get_weather_settings, requests_mock):
    ...
    with (
        ...
        patch(
            "apts.plotting.weather.plots.generate_plot_weather_summary"
        ) as mock_plot_weather_summary,
        patch(
            "apts.plotting.weather.plots.pyplot.subplots",
            return_value=(MagicMock(), MagicMock(shape=(7, 2))),
        ) as mock_subplots,
        patch("apts.plotting.weather.utils.mark_observation") as mock_mark_observation,
        patch(
            "apts.plotting.weather.utils.mark_good_conditions"
...
def test_plot_moon_illumination(mock_get_weather_settings, requests_mock):
    ...
    weather = Weather(lat=0, lon=0, local_timezone=pytz.utc)
    with patch("apts.utils.plot.PlotUtils.annotate_plot") as mock_annotate_plot:
